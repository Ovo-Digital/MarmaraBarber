...
export function AccountOrders({ customer }: AccountOrdersProps) {
  return (
    <div>
      <h2 className="mb-6 border-b border-[#e0e0e0] pb-4 text-[15px] font-semibold">Siparişlerim</h2>
        ...
        <>
          ...
          <ul>
            {customer.orders.map((order) => (
              ...
              >
                <div>
                  <p className="sm:hidden text-[10px] font-semibold uppercase text-[#999] mb-1">Sipariş No</p>
                  <p className="font-medium break-all">{formatOrderNumber(order.orderNumber)}</p>
                </div>
                <div>
                  <p className="sm:hidden text-[10px] font-semibold uppercase text-[#999] mb-1">Durum</p>
                  <p>{formatOrderStatus(order.financialStatus, order.fulfillmentStatus)}</p>
                </div>
                <div>
                  <p className="sm:hidden text-[10px] font-semibold uppercase text-[#999] mb-1">Tarih</p>
                  <p>{formatOrderDate(order.processedAt)}</p>
                </div>
                <div>
                  <p className="sm:hidden text-[10px] font-semibold uppercase text-[#999] mb-1">Toplam</p>
